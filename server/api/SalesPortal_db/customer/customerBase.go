 ...
 * 
 *  TO CUSTOMIZE customerBase.go PLEASE EDIT ./customer.go
 * 
...
type customer struct {
	...
	_customer	bson.ObjectId	`bson:"_customer,omitempty" json:"_customer"`
...
func (config *Config) create(writer http.ResponseWriter, req *http.Request) {
	...
	config.Database.C("customer").Insert(body)
...
func (config *Config) delete(writer http.ResponseWriter, req *http.Request) {
	...
	config.Database.C("customer").RemoveId(bson.ObjectIdHex(id))
...
func (config *Config) get(writer http.ResponseWriter, req *http.Request) {
	...
	var result customer
	id := chi.URLParam(req, "id")
	err := config.Database.C("customer").FindId(bson.ObjectIdHex(id)).One(&result)
	if err != nil {
		panic(err)
	}
...
func (config *Config) list(writer http.ResponseWriter, req *http.Request) {
	...
	var result []customer
	err := config.Database.C("customer").Find(nil).All(&result)
	if err != nil {
		panic(err)
	}
...
func (config *Config) update(writer http.ResponseWriter, req *http.Request) {
	...
	var body customer
	json.NewDecoder(req.Body).Decode(&body)
	config.Database.C("customer").UpdateId(bson.ObjectIdHex(id), body)
	render.JSON(writer, req, body)
}
