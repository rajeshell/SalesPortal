...
class ItemsList extends Component {
  ...
  // Load data on start
  componentWillMount() {
    this.props.actionsItems.loadItemsList();
  }
  ...
  confirmDialogDelete(id) {
    this.props.actionsItems.deleteItems(this.state.idDelete).then(data => {
      this.props.actionsItems.loadItemsList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right">ItemDescription</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/itemses/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.ItemsListReducer.listItems
  };
}
